...
double act_afb(int y1, int m1, int d1, 
               int y2, int m2, int d2) 
{   
    if( y1 == y2 ){
        double denom = (isaleapyear(y1) && (m1 < 3) ? 366e0 : 365e0);
        ...
        double denoma = (isaleapyear(y1) && (m1 < 3) ? 366e0 : 365e0);

        int diffb = date_to_jd(y2, m2, d2);
        diffb -= date_to_jd(y2, 1, 1);

        double denomb = (isaleapyear(y2) && (m2 >= 3) ? 366e0 : 365e0);
